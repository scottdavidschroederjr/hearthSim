...
// Set Selection //
        function setSelection() {
            if (set == "Classic Set") {
                ...
                setEpic = 27;
                setLegendary = 20;
            } else if (set == "Journey to UnGoro" || set == "Knights of the Frozen Throne" || set == "Kobolds & Catacombs" || set == "Rastakhan Rumble" || set == "Uldum") {
                setCommon = 49;
                setRare = 36;
                setEpic = 27;
                setLegendary = 23;
...
function selectRarity() {
    ...
    for (i = 0; cardPack.length < 5; i++) {
     roll = Math.random();
     let legendaryProb = (0.01127+1.17350*(1/(41-legendaryPityCounter)));
     let epicProb = (0.06305+1.03953*(1/(11-epicPityCounter)));
     let rareProb = ((0.229*(1-(legendaryProb+epicProb)))/0.955);
     let commonProb = (1-(legendaryProb+epicProb+rareProb));
...
function result() {
    ...
    if (resultsArray.length == sims) {
        let average = Math.round(((resultsArray.reduce(function(a,b){return a+b;}))/resultsArray.length));
        cost = Math.round(average*1.250);



        document.getElementById("resultsOutput").innerHTML=(set + " will take " + average + " packs to complete.");
        document.getElementById("costOutput").innerHTML=("Estimated Cost of Completion: $" + cost);
